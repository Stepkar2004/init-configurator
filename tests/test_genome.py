...
Every skill here is transferable: the repo's skill set equals the shipped set
exactly (a repo-only skill cannot creep back in). The root constitution
(CLAUDE.md/AGENTS.md) is per-project and generated by `beacons.py`, not shipped.
"""
...
class TestGenomeMatchesRepoSkills:
    ...
    def test_shipped_set_is_exactly_the_repo_skill_set(self) -> None:
        """No repo-only skill can creep back, and nothing ships that the repo lacks."""
        shipped = {entry.name for entry in genome_root().joinpath("skills").iterdir()}
        repo = {entry.name for entry in REPO_SKILLS.iterdir() if entry.is_dir()}
        assert shipped == set(SHIPPED_SKILLS) == repo
