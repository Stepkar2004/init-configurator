...
PRE_COMMIT_CONFIG = """\
# Generated by init-configurator. Run `pre-commit install` once and every commit
# is checked from then on. Add your own hooks below - init never overwrites this.
repos:
  - repo: local
    hooks:
      - id: lint-paths
